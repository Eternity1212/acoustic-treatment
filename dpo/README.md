...
`config_dpo.yaml`，再通过 `train_dpo.py` 这个轻量入口启动训练。

## 文件说明

- `train_dpo.py`：命令行入口，负责读取 YAML 配置并启动训练
...
- `config_dpo.yaml`：默认路径和关键超参数配置文件
...
- `early_stopping_enabled` / `early_stopping_patience` / `early_stopping_min_delta`：控制基于验证集 `preference_accuracy` 的早停行为
- 其余训练与模型超参数

YAML 文件是主配置来源，少量命令行参数可以覆盖其中的字段。
...
若验证集 `preference_accuracy` 连续若干轮没有超过历史最佳值加上 `early_stopping_min_delta`，
并且达到 `early_stopping_patience`，训练会提前结束。
